...
DRACUT_PATH = '/lib/dracut/dracut.conf.d/10-nvidia.conf'
...
options nvidia-current NVreg_UsePageAttributeTable=1
...
MODESET_CURRENT_S0ix = '''# Automatically generated by EnvyControl
...
def graphics_mode_switcher(graphics_mode, user_display_manager, enable_force_comp, coolbits_value, rtd3_value, enable_s0ix, use_nvidia_current):
    ...
    if graphics_mode == 'integrated':
        ...
        if enable_s0ix == None:
            ...
            # setup S0ix
            if use_nvidia_current:
                create_file(MODESET_PATH, MODESET_CURRENT_S0ix)
            else:
...
def rebuild_initramfs():
    ...
    # LunaOS with dracut
    elif os.path.exists('/etc/lunaos-release') and os.path.exists('/usr/bin/dracut-rebuild'):
        command = ['dracut-rebuild']
    else:
        command = []
...
def main():
    ...
    parser.add_argument('--s0ix', action='store_true',
                        help='Enable S0ix-based power management on Hybrid mode')
    parser.add_argument('--use-nvidia-current', action='store_true',
                        help='Use Nvidia Current modules in Debian')
